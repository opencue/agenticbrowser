...
- **Watching another agent's space** → `taskSpaces.observe(nameOrId)`, never `takeOver`. Observing attaches read-only: snapshot, screenshot and the read helpers work, every mutating call throws until you `takeOver`. `takeOver` does not join a space, it seizes one — the agent already working there keeps driving, so you both act on the same page and neither knows. The guarantee is cooperative, not enforced; see the reference.
