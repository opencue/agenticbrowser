...
const TEST_ENV = {
  ...
  // The badge names whichever harness is driving, so without a fixed name these
  // assertions would pass under Claude and fail under codex — the suite would
...
describe("ego-browser Linux port", () => {
  it("observes a page: navigation, tabs, refs and nested iframes", async () => {
    ...
    assert.match(
      out,
      /TITLE:\s+ego linux port fixture/,
      "page title read back",
    );
    assert.match(
      out,
      /TABS:\s+\d+ \(active: 1\)/,
      "exactly one tab reports active",
    );
    ...
    assert.match(
      out,
      /button "Increment counter" \[ref=\d+/,
      "button carries a ref",
    );
    assert.match(
      out,
      /link "Go to nav target".*url=\/nav-target/,
      "link exposes its href",
    );
    assert.match(
      out,
      /textbox "Your name".*loc=css:#name-input/,
      "input gets a stable locator",
    );
    ...
    assert.match(
      out,
      /button "Deep button" \[ref=\d+/,
      "element inside the deep iframe gets a ref",
    );

    // Layout-driven visibility, not a DOM dump.
    assert.doesNotMatch(
      out,
      /hidden text must not appear/,
  ...
  it("acts on a page: refs, locators, pointer input and screenshots", async () => {
    ...
    assert.match(
      out,
      /2\. @ref center:\s+\{"x":\d/,
      "@ref resolves to coordinates",
    );
    assert.match(
      out,
      /3\. after mouse click:\s+clicked/,
      "a synthesised click reaches the element",
    );
    assert.match(
      out,
      /5\. after locator click:\s+clicked/,
      "locator click reaches the element",
    );
    assert.match(
      out,
      /6\. locator fill:\s+Vikt/,
      "locator fill writes into the input",
    );
    assert.match(
      out,
      /7\. deep iframe ref:\s+\{"x":\d/,
      "a ref inside the deep iframe resolves",
    );
    assert.match(
      out,
  ...
  it("draws the agent's cursor without disturbing the page it acts on", async () => {
    ...
    assert.match(
      out,
      /1\. overlay present:\s+true/,
      "the overlay is injected on a click",
    );
    assert.match(
      out,
      /2\. cursor tracks click:\s+true/,
      "the cursor sits where the click landed",
    );
    assert.match(
      out,
      /3\. badge text:\s+Testbot · counting/,
      "the task state label is shown",
    );

    // The overlay must stay invisible to everything the harness relies on.
    assert.match(
      out,
      /4\. hit test at cursor:\s+click-button/,
      "elementFromPoint still sees the page",
    );
    assert.match(
      out,
      /5\. click still landed:\s+clicked/,
      "the click reached the element",
    );
    assert.match(
      out,
      /6\. overlay in snapshot:\s+false/,
      "the overlay is absent from the agent's snapshot",
    );
    assert.match(
      out,
      /7\. cursor held on wheel:\s+true/,
      "a scroll does not drag the cursor to (0, 0)",
    );
    ...
    assert.match(
      out,
      /8\. pressed on down:\s+true/,
      "the cursor holds pressed while the button is down",
    );
    assert.match(
      out,
      /9\. released on up:\s+true/,
      "and springs back once it is released",
    );

    // It marks an element, so it is anchored to the page rather than the screen.
    assert.match(
      out,
      /10\. travels with page:\s+true/,
      "the cursor scrolls with the element it is on",
    );

    // Shape and label both come from whatever sits under the cursor.
    assert.match(
      out,
      /11\. hand over a link:\s+hand/,
      "a link gets the hand, as the page itself asks",
    );
    assert.match(
      out,
      /12\. names what it is on:\s+Testbot · Go to nav target/,
      "the badge names it unprompted",
    );
    assert.match(
      out,
      /13\. beam over a field:\s+beam/,
      "a text field gets the beam",
    );
    ...
    assert.match(
      out,
      /14\. says it is typing:\s+Testbot · typing…/,
      "typing is announced",
    );
    assert.match(
      out,
      /15\. marks the field:\s+on/,
      "and the field being typed into is ringed",
    );
    assert.match(
      out,
      /16\. lets go when done:\s+true/,
      "the ring clears once the keystrokes stop",
    );

    // The highlighter — a marker drawn to explain something, not a selection.
    assert.match(
      out,
      /17\. highlight lines:\s+1/,
      "a phrase is found by its text and measured per line",
    );
    assert.match(out, /18\. bands drawn:\s+1/, "and gets a band");
    assert.match(
      out,
      /19\. note in badge:\s+Testbot · explaining this/,
      "the note says why",
    );
    assert.match(
      out,
      /20\. band matches text:\s+[0-2],[0-2]/,
      "the band sits on the text, within 2px",
    );
    assert.match(
      out,
      /21\. miss draws nothing:\s+true/,
      "text that is not there draws nothing",
    );
    ...
    assert.match(
      out,
      /23\. says what it reads:\s+Testbot · reading “.+”/,
      "the badge names the line being read",
    );
    assert.match(
      out,
      /24\. marks the lines:\s+true/,
      "and the lines it has passed are marked",
    );
    assert.match(
      out,
      /25\. input ends the read:\s+true/,
      "real input takes the cursor back from the sweep",
    );
    ...
    assert.match(
      out,
      /26\. trail:.*clicked Increment counter/,
      "a click is recorded by what it hit",
    );
    assert.match(
      out,
      /26\. trail:.*typed into Your name/,
      "named by the field's own label, not its placeholder",
    );
    assert.match(
      out,
      /26\. trail:.*highlighted explaining this/,
      "so is a highlight, by its note",
    );
    assert.match(
      out,
      /26\. trail:.*read Helper e2e fixture/,
      "and a read, by what it started on",
    );
    ...
    assert.match(
      out,
      /27\. cursor leaves view:\s+true/,
      "a long scroll carries the cursor off screen",
    );
    assert.match(
      out,
      /28\. badge stays in it:\s+true/,
      "but the badge stays legible",
    );
    assert.match(
      out,
      /29\. and points back:\s+↑/,
      "pointing back the way the cursor went",
    );
    ...
    assert.match(
      out,
      /30\. drawn independently:\s+true/,
      "and does not depend on the off-screen pointer layer",
    );
    assert.match(
      out,
  ...
  it("emulates task spaces with scoped tabs, ownership and lifecycle", async () => {
    ...
    assert.match(
      out,
      /1\. created:\s+\{"id":\d+.*"ownership":"agent"/,
      "a space is created agent-owned",
    );
    assert.match(
      out,
      /2\. alpha page:\s+ego linux port fixture/,
      "the agent works inside the space",
    );
    assert.match(
      out,
      /3\. second space: \{"id":\d+/,
      "a second, independent space is created",
    );
    assert.match(
      out,
      /4\. beta page:\s+about:blank#beta/,
      "the second space navigates on its own",
    );
    assert.match(
      out,
      /5\. back in alpha:ego linux port fixture/,
      "switching returns the agent to that space's page",
    );
    assert.match(
      out,
      /7\. after handOff:\["agentDelegatedToUser"/,
      "handOff flips ownership",
    );
    assert.match(
      out,
      /8\. after cleanup:\[\]/,
