...
import { COLLABORATION_REQUEST_FILE, TASK_SPACE_FILE } from "./paths.mjs";
...
export async function startSpacesServer(shim, options = {}) {
  ...
  const collaborationStore =
    options.collaborationStore ?? shim.collaborationStore ?? null;
    ...
    stateWatcher = watch(
      ...
      (_event, filename) => {
        if (
          !filename ||
          [
            basename(TASK_SPACE_FILE),
            basename(COLLABORATION_REQUEST_FILE),
  ...
  const handleRequest = async (request, response) => {
    ...
    if (
      request.method === "GET" &&
      url.pathname === "/api/collaboration/requests"
    ) {
      ...
      try {
        const requests = await collaborationStore.list({ view });
        const pendingCount = (
          view === "pending"
            ? requests
            : await collaborationStore.list({ view: "pending" })
        ).length;
        json(response, 200, {
          enabled: true,
          requests,
    ...
    const collaborationMatch =
      /^\/api\/collaboration\/requests\/([^/]+)\/(open|respond|archive)$/.exec(
        url.pathname,
      );
    if (request.method === "POST" && collaborationMatch) {
      if (!collaborationStore) {
        json(response, 404, { error: "collaboration inbox is disabled" });
        return;
      }
      ...
      // Request ids are UUIDs generated by the store. Keeping the path segment
      // encoded avoids a malformed percent escape turning a bad id into an
      // unhandled URIError before the endpoint's structured error handling.
      const requestId = collaborationMatch[1];
      const operation = collaborationMatch[2];
      try {
        if (operation === "open") {
          const current = await collaborationStore.get(requestId);
          if (!current) {
            json(response, 404, { error: "collaboration request not found" });
            return;
          }
          await ego.useTaskSpace(current.taskSpaceId);
          const presented = shim.presentTaskSpaceForPanel
            ? await shim.presentTaskSpaceForPanel(current.taskSpaceId)
            : await ego.presentTaskSpace(current.taskSpaceId);
          if (
            presented?.visible === false &&
            presented.reason === "no-live-tab"
          ) {
            json(response, 410, {
              error: "task space has no live tab",
              reason: "no-live-tab",
              request: current,
            });
            return;
          }
          const opened = await collaborationStore.open(requestId, {
            requestVersion: parsed.body.requestVersion,
          });
          json(response, 200, {
            request: opened,
            presented: presented?.visible === true,
            ...(presented || {}),
          });
          events.notify();
          return;
        ...
        if (operation === "respond") {
          const kind = parsed.body.response?.kind;
          const result =
            kind === "done" ? "done" : kind === "cancel" ? "cancel" : null;
          if (!result) {
            json(response, 422, {
              error: "manual response kind must be done or cancel",
            });
            return;
          }
          let answered = await collaborationStore.respond(requestId, {
            requestVersion: parsed.body.requestVersion,
            result,
          });
          if (result === "done" && answered.response?.resumed !== true) {
            try {
              await ego.takeOverTaskSpace(answered.taskSpaceId);
              answered = await collaborationStore.markResume(answered.id, {
                resumed: true,
                expectedResult: "done",
              });
            } catch {
          ...
          json(response, 200, {
            request: answered,
            accepted: true,
            resumed: answered.response?.resumed === true,
          });
          events.notify();
          return;
        }

        const archived = await collaborationStore.archive(requestId, {
          requestVersion: parsed.body.requestVersion,
        });
        json(response, 200, { request: archived });
        events.notify();
      } catch (error) {
        json(response, error?.status || 500, {
          error: error?.message || "collaboration request failed",
